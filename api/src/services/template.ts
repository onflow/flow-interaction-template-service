...
class TemplateService {
  ...
  async getTemplateByCadence(
    base64Cadence: string,
    network: string,
  ) {
  ...
  async seed() {
    ...
    for (let template of templates) {
      try {
        ...
        let mainnet_cadence = fcl.InteractionTemplateUtils.deriveCadenceByNetwork({
          template: parsedTemplate,
          network: "mainnet"
        })

        let testnet_cadence = fcl.InteractionTemplateUtils.deriveCadenceByNetwork({
          template: parsedTemplate,
          network: "testnet"
        })
